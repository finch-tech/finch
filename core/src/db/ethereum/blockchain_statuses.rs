...
use models::ethereum::{BlockchainStatus, BlockchainStatusPayload};
...
pub fn insert(
    ...
    use schema::eth_blockchain_statuses::dsl;

    insert_into(dsl::eth_blockchain_statuses)
        .values(&payload)
        .get_result(conn)
...
pub fn update(
    ...
    update(dsl::eth_blockchain_statuses.filter(dsl::network.eq(network)))
        .set(&payload)
        .get_result(conn)
        .map_err(|e| Error::from(e))
}
